...
The `@uppy/drag-drop` plugin allows users to drop files onto a specified area. Useful when you only want the local device as a file source, don’t need file previews and metadata editing UI, and using [`@uppy/dashboard`](/docs/dashboard/) would be too heavy.
...
A unique identifier for this plugin. Defaults to `'DragDrop'`. Use this if you need to add multiple DragDrop instances.
